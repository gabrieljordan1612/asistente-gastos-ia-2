        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    ...
                    className="bg-primary text-background font-bold py-2 px-4 rounded-lg hover:bg-yellow-500 transition-transform duration-300 hover:scale-105 shadow-lg shadow-primary/20 animate-glow"
                >
                    + Añadir Ingreso
                </button>
            </div>
            ...
            <div className="mb-6 flex flex-wrap items-end gap-4 p-4 bg-surface rounded-lg border border-border">
                ...
                <div>
                    ...
                    <input
                        ...
                        className="mt-1 bg-background border border-border rounded-md shadow-sm py-2 px-3 text-text-secondary focus:outline-none focus:ring-primary focus:border-primary"
                    />
                </div>
            </div>
                ...
                <table className="min-w-full divide-y divide-border">
                    ...
                    <tbody className="divide-y divide-border">
                        {filteredIncome.length > 0 ? filteredIncome.map((inc) => (
                            <tr key={inc.id} className="hover:bg-background/50 transition-colors">
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">{new Date(inc.date + 'T00:00:00').toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' })}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-right text-secondary">{formatCurrency(inc.amount)}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex items-center justify-end space-x-3">
                                        <button onClick={() => requestEditIncome(inc)} className="text-primary hover:text-yellow-400" aria-label={`Editar ingreso: ${inc.description || inc.source}`}>
                                        ...
                                        <button onClick={() => requestDeleteIncome(inc.id)} className="text-red-500 hover:text-red-400" aria-label={`Eliminar ingreso: ${inc.description || inc.source}`}>
